...
const PYTHON_API_URL = process.env.PYTHON_API_URL || "http://localhost:8000/api/classify";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file) forwardData.append("file", file);
    if (barcode) forwardData.append("barcode", barcode);
